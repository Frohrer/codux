...
COPY package-install-entrypoint.sh mkindex.sh serve.py /
RUN chmod +x package-install-entrypoint.sh
RUN chmod +x mkindex.sh
RUN find . -type f -name "*.sh" -exec chmod +x {} \;
ENTRYPOINT ["/package-install-entrypoint.sh"]
# CMD ["--no-build"]
EXPOSE 8000