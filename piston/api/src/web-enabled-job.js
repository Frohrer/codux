...
class WebEnabledJob extends Job {
    constructor(options) {
        const filteredDeps = options.dependencies ?
            options.dependencies.filter(dep => !dep.match(/^streamlit$/i)) :
    ...
    async installDependencies(box, event_bus = null) {
        ...
        const installResult = await this.safe_call(
            box,
            'packagemanager',
            args,
            this.timeouts.run,
    ...
    waitForStreamlitServer(event_bus) {
        ...
        return new Promise((resolve, reject) => {
            ...
            event_bus.on('stdout', (data) => {
                ...
                if (chunk.includes('You can now view your Streamlit app in your browser') ||
    ...
    async execute(box, event_bus = null) {
        ...
        if (isStreamlit) {
            ...
            this.args = [
                mainFile,
                '--server.baseUrlPath', proxyInfo.path,
                '--server.port', this.webAppPort.toString()
            ];
    ...
    isWebApp() {
        ...
        return this.files.some(file => 
            webFrameworks.some(framework => 
                file.content.includes(`import ${framework}`) ||
                file.content.includes(`from ${framework}`)
            )
        );
    }
}
