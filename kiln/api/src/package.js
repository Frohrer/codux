...
class Package {
    ...
    async runInEnv(command) {
        return new Promise((resolve, reject) => {
            const proc = cp.spawn(
                'bash',
                ['-c', `cd "${this.install_path}" && source environment && ${command}`],
                { stdio: 'inherit' }
            );
    ...
    async install() {
        ...
        if (fss.existsSync(this.install_path)) {
            logger.warn(
        ...
        logger.debug(
            `Downloading package from ${this.download} into ${this.install_path}`
        );
        const pkgpath = path.join(this.install_path, 'pkg.tar.gz');
        const download = await fetch(this.download);

        const file_stream = fss.createWriteStream(pkgpath);
        ...
        const read_stream = fss.createReadStream(pkgpath);
        await new Promise((resolve, reject) => {
        ...
        if (cs !== this.checksum) {
            throw new Error(
                `Checksum mismatch: expected ${this.checksum}, got ${cs}`
            );
        }

        logger.debug(
            `Extracting package files from archive ${pkgpath} into ${this.install_path}`
        );
